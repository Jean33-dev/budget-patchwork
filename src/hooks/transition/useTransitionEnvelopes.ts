...
/**
 * Hook to manage transition envelopes data
 */
export const useTransitionEnvelopes = () => {
  ...
  // Initialize envelopes from budgets and saved preferences
  useEffect(() => {
    ...
    if (savedPreferences && savedPreferences.length > 0) {
      ...
      const updatedEnvelopes = initialEnvelopes.map(env => {
        ...
        if (savedPref) {
          ...
          return {
            ...env,
            transitionOption: savedPref.transitionOption as any,
