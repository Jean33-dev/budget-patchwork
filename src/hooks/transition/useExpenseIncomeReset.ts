...
export const useExpenseIncomeReset = () => {
  const resetNonRecurringExpenses = async (dashboardId: string) => {
    ...
    console.log(`Suppression de ${nonRecurringExpenses.length} dépenses non récurrentes du dashboard ${dashboardId}`);
  ...
  const resetNonRecurringIncomes = async (dashboardId: string) => {
    ...
    console.log(`Suppression de ${nonRecurringIncomes.length} revenus non récurrents du dashboard ${dashboardId}`);
  ...
  const resetCategorySpent = async (categories: any[], dashboardId: string) => {
    ...
    console.log(`Réinitialisation des dépenses des catégories pour le dashboard ${dashboardId}`);
